...
exports.useUnsupportedBlocks = exports.useTaxonomies = void 0;
...
const mapToIHasNameAndId = (entities, path) => {
  return (entities || []).map(entity => ({
    ...entity,
    name: (0, _htmlEntities.decodeEntities)(getValueFromObjectPath(entity, path))
  }));
};
...
const usePostTypes = () => {
  ...
  const postTypeFormatSupportMap = (0, _element.useMemo)(() => {
    if (!postTypes?.length) {
      return {};
    }
    return postTypes.reduce((accumulator, type) => {
...
function useAllowedControls(attributes) {
  return (0, _data.useSelect)(select => select(_blocks.store).getActiveBlockVariation('core/query', attributes)?.allowedControls, [attributes]);
}
...
function useBlockNameForPatterns(clientId, attributes) {
  return (0, _data.useSelect)(select => {
    const activeVariationName = select(_blocks.store).getActiveBlockVariation('core/query', attributes)?.name;
...
const useUnsupportedBlocks = clientId => {
  return (0, _data.useSelect)(select => {
    ...
    getClientIdsOfDescendants(clientId).forEach(descendantClientId => {
       ...
       */
      const blockSupportsInteractivity = Object.is((0, _blocks.getBlockSupport)(blockName, 'interactivity'), true);
      const blockSupportsInteractivityClientNavigation = (0, _blocks.getBlockSupport)(blockName, 'interactivity.clientNavigation');
