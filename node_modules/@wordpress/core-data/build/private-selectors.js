...
const getHomePage = exports.getHomePage = (0, _data.createRegistrySelector)(select => (0, _data.createSelector)(() => {
  const canReadSiteData = select(_name.STORE_NAME).canUser('read', {
    kind: 'root',
    name: 'site'
  });
  if (!canReadSiteData) {
  ...
  const siteData = select(_name.STORE_NAME).getEntityRecord('root', 'site');
  if (!siteData) {
    return null;
  }
...
}, state => [(0, _selectors.canUser)(state, 'read', {
  kind: 'root',
  name: 'site'
